...
function buildTree(node, container, depth = 0, isLast = true, prefix = '') {{
    ...
    const saDot = node.sa ? ' <span style="color:#4f4;font-size:5px;vertical-align:middle;">●</span>' : '';
    const naDot = node.na ? ' <span style="color:#44f;font-size:5px;vertical-align:middle;">●</span>' : '';
    const htmlDots = node.at && node.at.length > 0 ? ' ' + '<span style="color:#fff;font-size:5px;vertical-align:middle;">●</span>'.repeat(node.at.length) : '';
...
const pNode = findNodeByPath(dataTree, partPath);
    ...
    const htmlDots = (pNode && pNode.at && pNode.at.length > 0) ? '<span style="color:#fff;font-size:9px;vertical-align:middle;margin-left:1px;">' + '●'.repeat(pNode.at.length) + '</span>' : '';
