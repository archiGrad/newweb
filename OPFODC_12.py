...
async function createThreeScene(container, images, node, highlightPath) {{
 ...
 let minX = Infinity, maxX = -Infinity;
    ...
    if (isLastStack) {{
        ...
        images.forEach(img => ssPathsSet.add(img.ss[1].replace('.ktx2', '.webp')));
        const ssPaths = [...ssPathsSet];
        images.forEach(img => {{
            const ssPath = img.ss[1].replace('.ktx2', '.webp');
            const sz = img.sz;
            ...
            if (img.anim && img.fm) {{
                ...
                ssSlotMap.push({{ ssPath, px: col * sz, py: row * sz, pw: sz, ph: sz }});
        ...
        ssPaths.forEach(ssPath => {{
            const img = document.createElement('img');
            img.src = ssPath;
            img.style.cssText = 'max-width:' + (80 / ssPaths.length) + '%;max-height:80%;object-fit:contain;image-rendering:pixelated;';
            img.dataset.ssPath = ssPath;
    ...
    function animate() {{
        ...
        if (isLastStack && _ssMouseDirty && ssBgContainer) {{
            ...
            if (_ssHits.length > 0 && _ssHits[0].uv) {{
                ...
                const ssKtx = mat._ssArray ? mat._ssArray[1] : null;
                const ssWebp = ssKtx ? ssKtx.replace('.ktx2', '.webp') : null;
                const px = uv.x * SPRITESHEET_SIZE;
                const py = (1 - uv.y) * SPRITESHEET_SIZE;
                let hitSlot = null;
                for (const slot of ssSlotMap) {{
                    if (slot.ssPath === ssWebp && px >= slot.px && px < slot.px + slot.pw && py >= slot.py && py < slot.py + slot.ph) {{
                        hitSlot = slot;
                        break;
                    }}
                }}
                const imgEl = ssWebp ? ssBgContainer.querySelector('img[data-ss-path="' + ssWebp + '"]') : null;
                if (hitSlot && imgEl) {{
