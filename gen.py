...
def scan_folder(path, ignore=['venv', '__pycache__', '.git', 'spritesheets', 'images']):
    ...
    noaccumulate_level = 0
    noaccumulate_file = path / '.noaccumulate'
    if noaccumulate_file.exists():
        try:
            content = noaccumulate_file.read_text().strip()
            if content.isdigit():
                noaccumulate_level = int(content)
            else:
                noaccumulate_level = float('inf')
            print(f"Scanning {path}: noaccumulate_level={noaccumulate_level}")
        except:
            noaccumulate_level = float('inf')
            print(f"Scanning {path}: noaccumulate_level=inf (error reading file)")
    else:
        print(f"Scanning {path}: no .noaccumulate file")
    
    if path.is_dir():
        for item in sorted(path.iterdir(), key=lambda x: (not x.is_dir(), x.name)):
            if item.name in ignore or item.name == '.noaccumulate':
                continue
            if item.is_file():
                if item.suffix.lower() in ['.png', '.jpg', '.jpeg', '.gif', '.webp']:
                    images.append(str(item.relative_to('.')))
                elif item.suffix.lower() == '.html':
                    text_path = str(item.relative_to('.'))
                    texts.append(text_path)
                    if noaccumulate_level > 0:
                        print(f"  Found text with noaccumulate level {noaccumulate_level}: {text_path}")
            elif item.is_dir():
        ...
        noaccumulate_texts_with_level = []
        
        if noaccumulate_level > 0:
            for text in texts:
                noaccumulate_texts_with_level.append({'path': text, 'level': noaccumulate_level})
        
        for child in children:
            all_images.extend(child['all_images'])
            
            for text in child['all_texts']:
                should_add = True
                for na_text in child.get('noaccumulate_texts_with_level', []):
                    if text == na_text['path'] and na_text['level'] > 1:
                        print(f"  Blocking accumulation of: {text} (level {na_text['level']-1} remaining)")
                        should_add = False
                        noaccumulate_texts_with_level.append({'path': na_text['path'], 'level': na_text['level'] - 1})
                        break
                if should_add:
                    all_texts.append(text)
        
        print(f"  Folder {path}: own_texts={len(texts)}, all_texts={len(all_texts)}")
    ...
    return {
        ...
        'own_images': images,
        'own_texts': texts,
        'noaccumulate_texts_with_level': noaccumulate_texts_with_level
    }
...
async function createThreeScene(container, images) {
    ...
    //random based on sede function to predict the zoomlevel of each 3d div.
   ...
   const randomZoom = 
    rand < 0.33 ? 0.1 :  //33% change of zoming in (fit to div * 0.1)
    rand < 0.66 ? 1 : //33% change of getting a zoom*1 (fit in box)
    2; //33%change of zooming in by 5
...
const updateCount = () => {
    ...
    countDiv.innerHTML = `<span style="cursor:pointer;padding:0 5px;user-select:none;color:#f44" id="nav-up">&#60;</span> zoom: ${randomZoom.toFixed(2)} | ${loadedStacks}/${totalStacks} stacks | ${loadedImages}/${totalImages} images <span style="cursor:pointer;padding:0 5px;user-select:none;color:${downColor}" id="nav-down">&#62;</span>`;
};
...
function getVisibleTexts(node, viewingNode) {
    console.log('DEBUG node structure:', {
        path: node.path,
        viewing: viewingNode.path,
        has_noaccumulate_texts_with_level: !!node.noaccumulate_texts_with_level,
        noaccumulate_data: node.noaccumulate_texts_with_level
    });
    
    const visible = new Set(node.all_texts.map(t => t.original_path || t));
    
    const pathDepth = (path) => path.split('/').length;
    
    function removeFromChildren(n) {
        if (n.noaccumulate_texts_with_level && n !== viewingNode) {
            n.noaccumulate_texts_with_level.forEach(item => {
                const textOriginDepth = pathDepth(n.path);
                const viewingDepth = pathDepth(viewingNode.path);
                const levelsUp = textOriginDepth - viewingDepth;
                
                if (levelsUp > 0 && levelsUp <= item.level) {
                    console.log(`Node ${node.path}: removing text ${item.path} (level=${item.level}, levelsUp=${levelsUp}, from ${n.path})`);
                    visible.delete(item.path);
                } else {
                    console.log(`Node ${node.path}: keeping text ${item.path} (level=${item.level}, levelsUp=${levelsUp})`);
                }
            });
        }
        n.children.forEach(removeFromChildren);
    }
    
    removeFromChildren(node);
    
    const result = Array.from(visible);
    console.log(`Node ${node.path}: visible texts = ${result.length}`, result);
    return result;
}
...
async function renderContent(node) {
    ...
    const visibleTexts = getVisibleTexts(node, node);
    console.log('=== renderContent for', node.path, '===');
    console.log('all_texts:', node.all_texts.length);
    console.log('visibleTexts:', visibleTexts.length);
    ...
    const allChildren = node.children.length > 0 ? node.children : [node];
    const children = allChildren.filter(child => {
        const childVisibleTexts = getVisibleTexts(child, node);
        const hasVisibleTexts = childVisibleTexts.length > 0;
        const hasImages = child.all_images.length > 0;
        return hasImages || hasVisibleTexts;
    });
    
    console.log(`Filtered children: ${children.length} out of ${allChildren.length}`);
    ...
    for (const child of children) {
        ...
        console.log(`Child ${child.path}: hasImages=${hasImages}, hasVisibleTexts=${hasVisibleTexts}`);

    if (!hasImages && !hasVisibleTexts) {
        console.log(`Skipping empty child: ${child.path}`);
        continue;
    }
    ...
    if (hasImages && hasVisibleTexts) {
        ...
        let htmlContent = '';
        for (const path of childVisibleTexts) {
            htmlContent += await loadText(path);
        }
        
        const hasParent = currentNode.path.split('/').length > 1;
        
        const firstChildWithText = currentNode.children?.find(c => getVisibleTexts(c, currentNode).length > 0);
        ...
        navLeft.onclick = () => {
            if (currentNode && currentNode.path) {
                const parentPath = currentNode.path.split('/').slice(0, -1).join('/');
                const parentNode = findNodeByPath(dataTree, parentPath) || dataTree;
                if (parentNode && getVisibleTexts(parentNode, parentNode).length > 0) {
...
if (firstChildWithText) {
        ...
        navLeft.onclick = () => {
            if (currentNode && currentNode.path) {
                const parentPath = currentNode.path.split('/').slice(0, -1).join('/');
                const parentNode = findNodeByPath(dataTree, parentPath) || dataTree;
                if (parentNode && getVisibleTexts(parentNode).length > 0) {
                    renderContent(parentNode);
                }
            }
        };
        ...
        navRight.onclick = () => {
            if (currentNode && currentNode.children && currentNode.children.length > 0) {
                const firstChildWithText = currentNode.children.find(c => getVisibleTexts(c).length > 0);
                if (firstChildWithText) {
                    renderContent(firstChildWithText);
                }
            }
