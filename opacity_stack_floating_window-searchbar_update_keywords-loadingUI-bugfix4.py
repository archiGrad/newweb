...
function assembleHtmlParts(parts) {{
...
function buildTree(node, container, depth = 0, isLast = true, prefix = '') {{
    ...
    const htmlDots = node.at && node.at.length > 0 ? ' ' + buildHtmlDots(node.at, 5) : '';
...
const pNode = findNodeByPath(dataTree, partPath);
    ...
    const htmlDots = buildHtmlDots(pNode ? pNode.at : null, 9);
...
textDiv.innerHTML = assembleHtmlParts(htmlParts);
