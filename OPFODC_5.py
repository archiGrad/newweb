...
if PROCESS_IMG_LOWRES:
    DEFAULTS['SPRITE_SIZE'] = 128
    DEFAULTS['LABEL_SPRITE_SIZE'] = 128
...
const updateCount = () => {{
            ...
            for (let i = 0; i < stackImages.length; i++) {{
                ...
                if (imgData.anim) {{
                    ...
                    mesh.onBeforeRender = function() {{
                        const frame = Math.floor(Date.now() / this.userData.imgData.gd) % this.userData.imgData.fm.length;
                        const idx = this.userData.imgData.fm[frame];
    ...
    const sceneData = {{ scene, renderer, camera, controls, animationId: null, resizeHandler, labelContainer, countDiv, stackGroups }};
