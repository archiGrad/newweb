...
async function createThreeScene(container, images, node) {{
    ...
    console.log('[createThreeScene] images:', images.length, 'threshold:', QUICKLOAD_THRESHOLD, 'useInstantLoad:', useInstantLoad, 'delay:', delay);
    ...
    function focusStack(folderName) {{
        for (const [folder, data] of Object.entries(stackGroups)) {{
            data.material.opacity = (folder === folderName) ? 1.0 : 0.5;
        }}
    }}
    ...
    const loadingPromise = (async () => {{
        ...
        if (node.grid_layout) {{
            for (let stackIdx = 0; stackIdx < folders.length; stackIdx++) {{
                ...
                for (let i = 0; i < stackImages.length; i++) {{
                    ...
                    if (!stackMaterial) {{
                        stackMaterial = getStackMaterial(imgData.ss, folderName);
                    }}
                    
                    const aspect = imgData.w / imgData.h;
                    const height = 1.5;
                    const width = height * aspect;
                    const mesh = new THREE.Mesh(sharedGeometry, stackMaterial);
                    ...
                    updateCount();
                    progress.imgs++;
                    if (progress.imgs % LOADINGSCREEN_IMG_INCREMENT === 0 || progress.imgs === progress.imgsTotal) await updateLoaderAsync();
                }}
                ...
                label.querySelectorAll('span[data-path]').forEach(span => {{
                    span.onclick = (e) => {{
                        ...
                        if (clickedNode && clickedNode.path === currentNode.path) {{
                            focusStack(folderName);
                        }} else if (clickedNode) {{
                            renderContent(clickedNode);
                        }}
                    }};
                }});
            }}
        ...
        console.log('[createThreeScene] FINISHED loading all stacks for:', node.name);
    }})();
    ...
    const sceneData = {{ scene, renderer, camera, controls, animationId: null, resizeHandler, stats, labelContainer, countDiv, stackGroups, focusStack, unfocusAll }};
    ...
    function animate() {{
        ...
        if (frameCount % 10 === 0) {{
            ...
            stackLabels.forEach(({{ element, position, xPos, zPos }}, idx) => {{
                ...
                const meshes = scene.children.filter(c => c.type === 'Group').flatMap(g => g.children);
...
async function renderContent(node) {{
    ...
    console.log('[renderContent] Waiting for layout...');
    await new Promise(resolve => requestAnimationFrame(resolve));
    console.log('[renderContent] Layout complete, creating scenes...');

    const sceneDivs = Array.from(contentDiv.children).filter(d => d._sceneContainer);
    for (const div of sceneDivs) {{
        console.log('[renderContent] Creating scene for container size:', div._sceneContainer.clientWidth, 'x', div._sceneContainer.clientHeight);
        scenePromises.push(createThreeScene(div._sceneContainer, div._sceneChild.ai, div._sceneChild));
    }}

    console.log('[renderContent] Waiting for', scenePromises.length, 'scenes to load...');
    await Promise.all(scenePromises);
    console.log('[renderContent] All scenes loaded for node:', node.name);
