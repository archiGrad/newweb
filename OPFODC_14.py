...
function disposeScene(sceneData) {{
    ...
    if (sceneData.ssBgContainer && sceneData.ssBgContainer.parentNode) {{
        sceneData.ssBgContainer.parentNode.removeChild(sceneData.ssBgContainer);
    }}
...
async function createThreeScene(container, images, node, highlightPath) {{
 ...
 let minX = Infinity, maxX = -Infinity;
    ...
    let ssBgContainer = null;
    let ssHighlight = null;
    let ssInfoLabel = null;
    let ssImgWrapper = null;
    let ssSlotMap = [];
    let ssMeshCache = null;
    let ssWireframe = null;
    if (isLastStack) {{
        renderer.setClearColor(0x000000, 0);
        const ssPathsSet = new Set();
        images.forEach(img => ssPathsSet.add(img.ss[0].replace('_lod0.ktx2', '')));
        const ssKeys = [...ssPathsSet];
        images.forEach(img => {{
            const ssKey = img.ss[0].replace('_lod0.ktx2', '');
            const sz = img.sz;
            const spr = Math.floor(SPRITESHEET_SIZE / sz);
            if (img.anim && img.fm) {{
                const idx = img.fm[0];
                const col = idx % spr;
                const row = Math.floor(idx / spr);
                ssSlotMap.push({{ ssPath: ssKey, px: col * sz, py: row * sz, pw: sz, ph: sz }});
            }} else if (img.idx !== undefined) {{
                const col = img.idx % spr;
                const row = Math.floor(img.idx / spr);
                ssSlotMap.push({{ ssPath: ssKey, px: col * sz, py: row * sz, pw: sz, ph: sz }});
            }}
        }});
        ssBgContainer = document.createElement('div');
        ssBgContainer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;z-index:0;pointer-events:none;';
        ssImgWrapper = document.createElement('div');
        ssImgWrapper.className = 'ss-img-wrapper';
        ssKeys.forEach(ssKey => {{
            const imgEl = document.createElement('img');
            imgEl.src = ssKey + '_lod2.webp';
            imgEl.className = 'ss-bg-img';
            imgEl.dataset.ssKey = ssKey;
            imgEl._lodLevel = 2;
            ssImgWrapper.appendChild(imgEl);
            const lod1 = new Image();
            lod1.onload = () => {{ if (imgEl._lodLevel > 1) {{ imgEl.src = lod1.src; imgEl._lodLevel = 1; }} }};
            lod1.src = ssKey + '_lod1.webp';
            const lod0 = new Image();
            lod0.onload = () => {{ if (imgEl._lodLevel > 0) {{ imgEl.src = lod0.src; imgEl._lodLevel = 0; }} }};
            lod0.src = ssKey + '_lod0.webp';
        }});
        ssHighlight = document.createElement('div');
        ssHighlight.className = 'ss-highlight';
        ssInfoLabel = document.createElement('div');
        ssInfoLabel.className = 'ss-info-label';
        ssImgWrapper.appendChild(ssHighlight);
        ssImgWrapper.appendChild(ssInfoLabel);
        ssBgContainer.appendChild(ssImgWrapper);
        container.appendChild(ssBgContainer);
        const ssSizeImages = () => {{
            const cw = container.clientWidth * 0.8;
            const ch = container.clientHeight * 0.8;
            const n = ssKeys.length;
            const gap = (n - 1) * 4;
            const maxByWidth = (cw - gap) / n;
            const maxByHeight = ch;
            const sz = Math.floor(Math.min(maxByWidth, maxByHeight));
            ssBgContainer.querySelectorAll('.ss-bg-img').forEach(img => {{
                img.style.width = sz + 'px';
                img.style.height = sz + 'px';
            }});
        }};
        ssSizeImages();
        new ResizeObserver(ssSizeImages).observe(container);
    }}
    ...
    const _ssMouse = new THREE.Vector2(-999, -999);
    const _ssRaycaster = new THREE.Raycaster();
    let _ssMouseDirty = false;
    if (isLastStack) {{
        renderer.domElement.addEventListener('mousemove', (e) => {{
            const rect = renderer.domElement.getBoundingClientRect();
            _ssMouse.x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
            _ssMouse.y = -((e.clientY - rect.top) / rect.height) * 2 + 1;
            _ssMouseDirty = true;
        }});
        renderer.domElement.addEventListener('mouseleave', () => {{
            _ssMouseDirty = false;
            if (ssHighlight) ssHighlight.style.display = 'none';
            if (ssInfoLabel) ssInfoLabel.style.display = 'none';
            if (ssWireframe) ssWireframe.visible = false;
        }});
    }}
...
const updateCount = () => {{
    ...
    const sceneData = {{ scene, renderer, camera, controls, animationId: null, resizeHandler, labelContainer, countDiv, stackGroups, ssBgContainer }};
    ...
    function animate() {{
        ...
        if (isLastStack && _ssMouseDirty && ssBgContainer) {{
            _ssMouseDirty = false;
            if (!ssMeshCache) {{
                ssMeshCache = [];
                scene.traverse(obj => {{ if (obj.isMesh) ssMeshCache.push(obj); }});
            }}
            scene.updateMatrixWorld();
            _ssRaycaster.setFromCamera(_ssMouse, camera);
            const _ssHits = _ssRaycaster.intersectObjects(ssMeshCache);
            if (_ssHits.length > 0 && _ssHits[0].uv) {{
                const hit = _ssHits[0];
                const uv = hit.uv;
                const mat = hit.object.material;
                const ssKey = mat._ssArray ? mat._ssArray[0].replace('_lod0.ktx2', '') : null;
                const px = uv.x * SPRITESHEET_SIZE;
                const py = (1 - uv.y) * SPRITESHEET_SIZE;
                let hitSlot = null;
                for (const slot of ssSlotMap) {{
                    if (slot.ssPath === ssKey && px >= slot.px && px < slot.px + slot.pw && py >= slot.py && py < slot.py + slot.ph) {{
                        hitSlot = slot;
                        break;
                    }}
                }}
                const imgEl = ssKey ? ssBgContainer.querySelector('img[data-ss-key="' + ssKey + '"]') : null;
                if (hitSlot && imgEl) {{
                    const imgRect = imgEl.getBoundingClientRect();
                    const wrapRect = ssImgWrapper.getBoundingClientRect();
                    const scale = imgRect.width / SPRITESHEET_SIZE;
                    ssHighlight.style.display = 'block';
                    ssHighlight.style.left = (imgRect.left - wrapRect.left + hitSlot.px * scale) + 'px';
                    ssHighlight.style.top = (imgRect.top - wrapRect.top + hitSlot.py * scale) + 'px';
                    ssHighlight.style.width = (hitSlot.pw * scale) + 'px';
                    ssHighlight.style.height = (hitSlot.ph * scale) + 'px';
                    const u0 = (hitSlot.px / SPRITESHEET_SIZE).toFixed(3);
                    const v0 = (hitSlot.py / SPRITESHEET_SIZE).toFixed(3);
                    const u1 = ((hitSlot.px + hitSlot.pw) / SPRITESHEET_SIZE).toFixed(3);
                    const v1 = ((hitSlot.py + hitSlot.ph) / SPRITESHEET_SIZE).toFixed(3);
                    ssInfoLabel.textContent = ssKey.split('/').pop() + '  uv [' + u0 + ',' + v0 + '] [' + u1 + ',' + v1 + ']  px [' + hitSlot.px + ',' + hitSlot.py + '] ' + hitSlot.pw + 'x' + hitSlot.ph;
                    ssInfoLabel.style.display = 'block';
                }} else {{
                    ssHighlight.style.display = 'none';
                    ssInfoLabel.style.display = 'none';
                }}
                const geo = hit.object.geometry;
                const pos = geo.attributes.position;
                const fi = hit.faceIndex;
                const qi = Math.floor(fi / 2);
                const vs = qi * 4;
                const order = [0, 1, 3, 2];
                const pts = [];
                for (let i = 0; i < 4; i++) {{
                    pts.push(pos.getX(vs + order[i]), pos.getY(vs + order[i]), pos.getZ(vs + order[i]));
                }}
                pts.push(pts[0], pts[1], pts[2]);
                if (!ssWireframe) {{
                    const wGeo = new LineGeometry();
                    wGeo.setPositions(pts);
                    const wMat = new LineMaterial({{ color: 'rgb(0,0,255)', linewidth: 3 }});
                    wMat.resolution.set(container.clientWidth, container.clientHeight);
                    ssWireframe = new Line2(wGeo, wMat);
                    ssWireframe.renderOrder = 999;
                    ssWireframe.frustumCulled = false;
                    ssWireframe.computeLineDistances();
                    hit.object.parent.add(ssWireframe);
                }} else {{
                    ssWireframe.geometry.setPositions(pts);
                    ssWireframe.computeLineDistances();
                    ssWireframe.material.resolution.set(container.clientWidth, container.clientHeight);
                }}
                ssWireframe.visible = true;
            }} else {{
                if (ssHighlight) ssHighlight.style.display = 'none';
                if (ssInfoLabel) ssInfoLabel.style.display = 'none';
                if (ssWireframe) ssWireframe.visible = false;
