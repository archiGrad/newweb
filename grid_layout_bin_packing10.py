...
fetch('data.json')
    ...
    .then(d => {{
        ...
        loadScreen.id = 'global-loader';
        loadScreen.innerHTML = 'Loading spritesheets: 0/0';
        loadScreen.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;background:black;color:white;display:flex;align-items:center;justify-content:center;font-family:monospace;font-size:20px;z-index:10000';
        document.body.appendChild(loadScreen);
        
        function collectAllSpritesheets(node) {{
            node.ai.forEach(img => {{
                if (!allSpritesheets.includes(img.ss)) {{
                    allSpritesheets.push(img.ss);
                }}
            }});
            node.children.forEach(child => collectAllSpritesheets(child));
        }}
        
        collectAllSpritesheets(dataTree);
        console.log(`[GLOBAL] Preloading ${{allSpritesheets.length}} spritesheets`);
        
        let loaded = 0;
        Promise.all(allSpritesheets.map(async (sheet) => {{
            await loadSpritesheet(sheet);
            loaded++;
            loadScreen.innerHTML = `Loading spritesheets: ${{loaded}}/${{allSpritesheets.length}}`;
        }})).then(() => {{
            console.log('[GLOBAL] All spritesheets loaded');
            loadScreen.remove();
        ...
        if (node.grid_layout) {{
            for (let stackIdx = 0; stackIdx < folders.length; stackIdx++) {{
                ...
                for (let i = 0; i < stackImages.length; i++) {{
                    ...
                    const texture = spritesheets[imgData.ss];
