...
async function createThreeScene(container, images, node) {
    ...
    function collectGridChildren(n) {
        if (n.grid_layout && n.all_images.length > 0) {
    ...
    const ungroupedFolders = folders.filter(f => !processedFolders.has(f));
    ...
    if (gridGroups.length > 0) {
        ...
        actualCols = maxGridX - minGridX + 1;
        actualRows = maxGridZ - minGridZ + 1;
    } else if (node.grid_layout) {
        actualCols = cols;
        actualRows = rows;
    } else {
    ...
    const gridInfoDiv = document.createElement('div');
    gridInfoDiv.style.position = 'absolute';
    gridInfoDiv.style.bottom = '25px';
    gridInfoDiv.style.right = '5px';
    gridInfoDiv.style.color = 'white';
    gridInfoDiv.style.fontFamily = 'monospace';
    gridInfoDiv.style.fontSize = '9px';
    gridInfoDiv.style.textAlign = 'right';
    gridInfoDiv.style.lineHeight = '1.2';
    gridInfoDiv.style.opacity = '0.7';
    container.appendChild(gridInfoDiv);

    if (gridGroups.length > 0) {
        const individualGrids = gridGroups.map(g => `${g.cols}x${g.rows}`).join('<br>');
        gridInfoDiv.innerHTML = individualGrids + '<br>―――<br>' + `${actualCols}x${actualRows}`;
    } else if (node.grid_layout) {
        gridInfoDiv.innerHTML = `${actualCols}x${actualRows}`;
    } else {
        gridInfoDiv.innerHTML = `${actualCols}x${actualRows}`;
    }
    ...
    const updateCount = () => {
        ...
        const gridInfo = actualCols && actualRows ? ` [${actualCols}x${actualRows}]` : '';
...
async function renderContent(node) {
    ...
    for (const child of children) {
        ...
        if (child.all_images.length > 0 && child.all_texts.length > 0) {
            ...
            setTimeout(() => createThreeScene(imgDiv, child.all_images, child), 0);

        } else if (child.all_images.length > 0) {
            setTimeout(() => createThreeScene(div, child.all_images, child), 0);
