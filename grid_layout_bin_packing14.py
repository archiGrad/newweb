...
let loadingProgress = {{
    spritesheets: {{ loaded: 0, total: 0 }},
    stacks: {{ loaded: 0, total: 0 }},
...
async function loadSpritesheet(path) {{
    if (spritesheets[path]) {{
        console.log('[loadSpritesheet] Using cached:', path);
        return spritesheets[path];
    }}
    if (pendingLoads[path]) return pendingLoads[path];
    
    console.log('[loadSpritesheet] Loading:', path);
    pendingLoads[path] = new Promise((resolve) => {{
        ...
        loader.load(path, (texture) => {{
            ...
            loadingProgress.spritesheets.loaded++;
            console.log('[loadSpritesheet] Loaded:', path, loadingProgress.spritesheets.loaded + '/' + loadingProgress.spritesheets.total);
            updateLoadingScreen();
            resolve(texture);
        }});
    }});
...
loadingProgress.stacks.total += folders.length;
loadingProgress.images.total += images.length;

const uniqueSpritesheets = new Set(images.map(img => img.ss));
loadingProgress.spritesheets.total += uniqueSpritesheets.size;
uniqueSpritesheets.forEach(ss => {{
    if (spritesheets[ss]) {{
        loadingProgress.spritesheets.loaded++;
    }}
}});

console.log('[createThreeScene] Totals for', node.name, '- SS:', uniqueSpritesheets.size, 'Stacks:', folders.length, 'Images:', images.length);
updateLoadingScreen();
...
console.log('[renderContent] Reset loading progress');
