...
async function loadSpritesheet(path) {
    ...
    async function createThreeScene(container, images) {
        ...
        const minZoom = 0.01;
        const maxZoom = 0.5;
        const randomZoom = minZoom + Math.random() * (maxZoom - minZoom);
...
const stackLabels = [];
    ...
    (async () => {
       ...
       for (let i = 0; i < stackImages.length; i++) {
            ...
            await new Promise(resolve => setTimeout(resolve, 1));
                ...
                label.innerHTML = `<span style="background:black;padding:2px 4px">${folder}</span><span style="background:black;padding:2px 4px;font-size:7px;margin-left:2px">${stackImages.length}</span>`;
                label.style.position = 'absolute';
                label.style.fontSize = '11px';
                labelContainer.appendChild(label);
                
                stackLabels.push({ element: label, position: worldPos });
    ...
    const updateCount = () => {
        countDiv.textContent = `${loadedStacks}/${totalStacks} stacks | ${loadedImages}/${totalImages} images`;
    };
    updateCount();
    ...
    const sceneData = { scene, renderer, camera, controls, animationId: null, resizeHandler };
    ...
    function animate() {
        ...
        stackLabels.forEach(({ element, position }) => {
...
async function renderContent(node) {
    ...
    for (const child of children) {
        ...
        if (child.all_images.length > 0 && child.all_texts.length > 0) {
            ...
            textLabel.className = 'div-label';
            textLabel.style.position = 'absolute';
            textLabel.style.top = '5px';
            textLabel.style.left = '5px';
            textLabel.style.background = 'black';
            textLabel.style.padding = '2px 5px';
            textLabel.style.zIndex = '100';
            
            const textParts = child.path.split('/');
            textParts.forEach((part, idx) => {
                if (idx > 0) {
                    const separator = document.createElement('span');
                    separator.textContent = ' > ';
                    textLabel.appendChild(separator);
                }
                ...
                link.className = 'tree-link';
                link.onclick = () => {
                    const targetPath = textParts.slice(0, idx + 1).join('/');
                    const findNode = (node) => {
                    ...
                    if (targetNode) renderContent(targetNode);
                };
                textLabel.appendChild(link);
            });
