...
async function createThreeScene(container, images, node) {{
 ...
 let minX = Infinity, maxX = -Infinity;
    ...
    const loadingPromise = (async () => {{
        ...
        function nodeDeco(n) {{
            return spriteConfig.show_nav_accessories ? navDeco(n, 'dot-sm') : '';
        }}
        ...
        function renderMenuLevel(treeNode, activePath, depth) {{
            ...
            for (const key of keys) {{
                ...
                if (spriteConfig.show_nav_accessories && !hasKids && child.imageCount) deco += `<span class="nav-count">(${{child.imageCount}})</span>`;
        ...
        for (const [groupKey, gData] of Object.entries(groupLabelData)) {{
            ...
            if (spriteConfig.show_nav_accessories) navHtml += `<span class="nav-count">(${{totalImages}})</span>`;
            ...
            function hoverPair(trigger, target, showVal) {{
                ...
                const hide = () => {{ t = setTimeout(() => {{ if (showVal === 'visible') {{ target.style.visibility = 'hidden'; target.style.pointerEvents = 'none'; }} else target.style.display = 'none'; }}, spriteConfig.submenu_close_delay); }};
    ...
    function animate() {{
        ...
        if (frameCount % 15 === 0) {{
            ...
            let targetLod;
            if (viewHeight < 10) targetLod = 0;
            else if (viewHeight < 40) targetLod = 1;
            else targetLod = 2;

            for (const key in stackMaterialCache) {{
                const mat = stackMaterialCache[key];
                if (!mat._ssArray) continue;
                const best = getBestLod(mat._ssArray, targetLod);
                if (best && mat.map !== best) {{
                    mat.map = best;
