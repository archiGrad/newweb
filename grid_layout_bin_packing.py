...
async function createThreeScene(container, images, node) {
    ...
    if (gridGroups.length > 0) {
        ...
        gridGroups.forEach(group => {
            ...
            maxGridWidth = Math.max(maxGridWidth, Math.abs(pos.x) + group.cols);
            maxGridHeight = Math.max(maxGridHeight, Math.abs(pos.y) + group.rows);
        });
        
        maxGridWidth = Math.max(maxGridWidth, Math.abs(Math.min(...Array.from(occupiedGrid.keys()).map(k => parseInt(k.split(',')[0])))));
        maxGridHeight = Math.max(maxGridHeight, Math.abs(Math.min(...Array.from(occupiedGrid.keys()).map(k => parseInt(k.split(',')[1])))));
    }
    ...
    } else if (gridGroups.length > 0) {
        offsetX = maxGridWidth * spacing / 2;
        offsetZ = maxGridHeight * spacing / 2;
    } else {
