...
async function renderContent(node) {
    ...
    for (const child of children) {
        ...
        if (child.all_images.length > 0 && child.all_texts.length > 0) {
            ...
            div.innerHTML = htmlContent + '<style>.div-label { position: absolute; top: 5px; left: 5px; background: black; padding: 2px 5px; border: 1px solid white; z-index: 100; }</style><div class="div-label">' + child.name + '</div>';
        }        
