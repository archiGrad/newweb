...
async function renderContent(node) {
    ...
    for (const child of children) {
        ...
        if (child.all_images.length > 0 && child.all_texts.length > 0) {
           ...
           div.innerHTML = htmlContent + '<style>.div-label { position: absolute; top: 5px; left: 5px; background: black; padding: 2px 5px; border: 1px solid white; z-index: 100; pointer-events: auto; }</style><div class="div-label">' + labelHtml + '</div>';
           const textLabel = div.querySelector('.div-label');
           textLabel.querySelectorAll('span[data-path]').forEach(span => {
               console.log('Text div - attaching to:', span.dataset.path);
               span.onclick = (e) => {
                   console.log('Text div clicked:', span.dataset.path);
                   e.stopPropagation();
                   const node = findNodeByPath(dataTree, span.dataset.path);
                   if (node) renderContent(node);
               };
            ...
            const coloredPath = child.path.split('/').map((part, idx, arr) => {
