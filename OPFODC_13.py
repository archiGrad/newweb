...
async function createThreeScene(container, images, node, highlightPath) {{
 ...
 let minX = Infinity, maxX = -Infinity;
    ...
    if (isLastStack) {{
        ...
        imgWrapper.style.cssText = 'display:flex;gap:4px;align-items:center;justify-content:center;';
        ssKeys.forEach(ssKey => {{
            const imgEl = document.createElement('img');
            imgEl.src = ssKey + '_lod2.webp';
            imgEl.style.cssText = 'max-width:' + (80 / ssKeys.length) + '%;max-height:80%;object-fit:contain;image-rendering:pixelated;';
        ...
        ssHighlight.style.cssText = 'position:absolute;border:2px solid #44f;pointer-events:none;display:none;z-index:1;box-sizing:border-box;';
        ssBgContainer.appendChild(imgWrapper);
        ssBgContainer.appendChild(ssHighlight);
        ...
        if (isLastStack && _ssMouseDirty && ssBgContainer) {{
            ...
            if (_ssHits.length > 0 && _ssHits[0].uv) {{
                ...
                if (hitSlot && imgEl) {{
                    ...
                    ssHighlight.style.left = (imgRect.left - containerRect.left + hitSlot.px * scale) + 'px';
                    ssHighlight.style.top = (imgRect.top - containerRect.top + hitSlot.py * scale) + 'px';
